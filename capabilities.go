...
func (c *Client) Capabilities() ai.Capabilities {
	return ai.Capabilities{
		// This provider enforces JSON but has no schema field: a schema is spelled\n\t\t// out in the prompt, so it is asked for rather than held to.
		Format: ai.FormatCapability{
			JSON:       ai.FormatNative,
			JSONSchema: ai.FormatEmulated,
			Strict:     ai.FormatEmulated,
		},
